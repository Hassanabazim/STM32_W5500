...
extern  MRCC_CFG RCC_PORT[MRCC_PREPHERIALS_NUM];
/**********************************************************************************************************************
 *  GLOBAL FUNCTIONS
 *********************************************************************************************************************/
...
ErrorState_t MRCC_enPeripheralInit(void)
{
	...
	for (u8 i = 0; i < MRCC_PREPHERIALS_NUM; i++)
	{
		if (RCC_PORT[i].State == ENABLE)
		{
			if (RCC_PORT[i].PeripheialID < MRCC_AHB_REG)
				SET_BIT(RCC->RCC_AHBENR, RCC_PORT[i].PeripheialID);

			else if (RCC_PORT[i].PeripheialID < MRCC_APB1_REG)
			{
				RCC_PORT[i].PeripheialID -= MRCC_AHB_REG;
				SET_BIT(RCC->RCC_APB1ENR, RCC_PORT[i].PeripheialID);
			}
			else if (RCC_PORT[i].PeripheialID < MRCC_APB2_REG)
			{
				RCC_PORT[i].PeripheialID -= MRCC_APB1_REG;
				SET_BIT(RCC->RCC_APB2ENR, RCC_PORT[i].PeripheialID);
			}
			else
			{
...
ErrorState_t MRCC_enPeripheralDeInit(void)
{
	...
	for (u8 i = 0; i < MRCC_PREPHERIALS_NUM ; i++)
	{
		...
		if (RCC_PORT[i].PeripheialID < MRCC_AHB_REG)
			CLR_BIT(RCC->RCC_AHBENR, RCC_PORT[i].PeripheialID);

		else if (RCC_PORT[i].PeripheialID < MRCC_APB1_REG)
		{
			RCC_PORT[i].PeripheialID -= MRCC_AHB_REG;
			CLR_BIT(RCC->RCC_APB1ENR, RCC_PORT[i].PeripheialID);
		}

		else if (RCC_PORT[i].PeripheialID < MRCC_APB2_REG)
		{
			RCC_PORT[i].PeripheialID -= MRCC_APB1_REG;
			CLR_BIT(RCC->RCC_APB2ENR, RCC_PORT[i].PeripheialID);
		}
