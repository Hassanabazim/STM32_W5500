...
extern  MGPIO_CFG_t GPIO_PORT[MGPIO_PINS_NUM];
...
ErrorState_t MGPIO_enInit(void)
{
	...
	for (u8 i = 0; i < MGPIO_PINS_NUM; i++)
	{
		switch(GPIO_PORT[i].GPIO_PORT)
		{
		case PORTA:
			if (GPIO_PORT[i].GPIO_PIN < MGPIO_CRL_MAX)
			{
				GPIOA->GPIO_CRL &= ~(MGPIO_MASK << (GPIO_PORT[i].GPIO_PIN * MGPIO_OFFSET)) ;
				GPIOA->GPIO_CRL |= GPIO_PORT[i].GPIO_DIR << (GPIO_PORT[i].GPIO_PIN * MGPIO_OFFSET) ;
			}
			else if(GPIO_PORT[i].GPIO_PIN < MGPIO_CRH_MAX)
			{
				GPIO_PORT[i].GPIO_PIN -= MGPIO_CRL_MAX  ;
				GPIOA->GPIO_CRH &= ~(MGPIO_MASK << (GPIO_PORT[i].GPIO_PIN * MGPIO_OFFSET));
				GPIOA->GPIO_CRH |= (GPIO_PORT[i].GPIO_DIR << (GPIO_PORT[i].GPIO_PIN * MGPIO_OFFSET));
			}
			else
			{
		...
		case PORTB:
			if (GPIO_PORT[i].GPIO_PIN < MGPIO_CRL_MAX)
			{
				GPIOB->GPIO_CRL &= ~(MGPIO_MASK << (GPIO_PORT[i].GPIO_PIN * MGPIO_OFFSET)) ;
				GPIOB->GPIO_CRL |= (GPIO_PORT[i].GPIO_DIR << (GPIO_PORT[i].GPIO_PIN * MGPIO_OFFSET)) ;
			}
			else if(GPIO_PORT[i].GPIO_PIN < MGPIO_CRH_MAX)
			{
				GPIO_PORT[i].GPIO_PIN -= MGPIO_CRL_MAX ;
				GPIOB->GPIO_CRH &= ~(MGPIO_MASK << (GPIO_PORT[i].GPIO_PIN * MGPIO_OFFSET)) ;
				GPIOB->GPIO_CRH |= (GPIO_PORT[i].GPIO_DIR << (GPIO_PORT[i].GPIO_PIN * MGPIO_OFFSET)) ;
			}
			else
			{
				local_state = OUT_OF_RANG_ERR;
		...
		case PORTC:
			if (GPIO_PORT[i].GPIO_PIN < MGPIO_CRL_MAX)
			{
				GPIOC->GPIO_CRL &= ~(MGPIO_MASK << (GPIO_PORT[i].GPIO_PIN * MGPIO_OFFSET)) ;
				GPIOC->GPIO_CRL |= (GPIO_PORT[i].GPIO_DIR << (GPIO_PORT[i].GPIO_PIN * MGPIO_OFFSET)) ;
			}
			else if(GPIO_PORT[i].GPIO_PIN < MGPIO_CRH_MAX)
			{
				GPIO_PORT[i].GPIO_PIN -= MGPIO_CRL_MAX ;
				GPIOC->GPIO_CRH &= ~(MGPIO_MASK) << (GPIO_PORT[i].GPIO_PIN * MGPIO_OFFSET) ;
				GPIOC->GPIO_CRH |= (GPIO_PORT[i].GPIO_DIR << (GPIO_PORT[i].GPIO_PIN * MGPIO_OFFSET)) ;
			}
			else
			{
				local_state = OUT_OF_RANG_ERR;
...
ErrorState_t MGPIO_enSetPortDirection(MGPIO_ID_t copy_u8CRID, MGPIO_PORT_t copy_u8port, MGPIO_DIR_t copy_u8Dir)
{
	...
	case PORTA:
		...
		{
			GPIOA->GPIO_CRL = copy_u8Dir * MGPIO_PORT_OFFSET ;
		}
		else if(copy_u8CRID == GPIO_CRH_ID)
		{
			GPIOA->GPIO_CRH = copy_u8Dir * MGPIO_PORT_OFFSET ;
		}
		else
		{
			local_state = OUT_OF_RANG_ERR;
	...
	case PORTB:
		if(copy_u8CRID == GPIO_CRL_ID)
		{
			GPIOB->GPIO_CRL = copy_u8Dir * MGPIO_PORT_OFFSET ;
		}
		else if(copy_u8CRID == GPIO_CRH_ID)
		{
			GPIOB->GPIO_CRH = copy_u8Dir * MGPIO_PORT_OFFSET ;
		}
		else
		{
			local_state = OUT_OF_RANG_ERR;
	...
	case PORTC:
		if(copy_u8CRID == GPIO_CRL_ID)
		{
			GPIOC->GPIO_CRL = copy_u8Dir * MGPIO_PORT_OFFSET ;
		}
		else if(copy_u8CRID == GPIO_CRH_ID)
		{
			GPIOC->GPIO_CRH = copy_u8Dir * MGPIO_PORT_OFFSET ;
		}
		else
		{
			local_state = OUT_OF_RANG_ERR;
